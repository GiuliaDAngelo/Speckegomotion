...
from functions.Speck_helpers import Specksetup, fetch_events
import matplotlib
import time
        ...
        with events_lock:
            if current_time - last_update_time > config.UPDATE_INTERVAL:
                if numevs[0] > 0:
                    ...
                    OMSneg_map, indexes_pos = egomotion(OMSneg, net_center, net_surround, config.DEVICE, config.MAX_Y,
                                                    config.MAX_X,
                                                    config.OMS_PARAMS['threshold'])
                    ...
                    cv2.imshow('OMS map', OMSpos_map)
                    cv2.imshow('OMS map', OMSneg_map)
